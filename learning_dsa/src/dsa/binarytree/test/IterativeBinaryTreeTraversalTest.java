...
import dsa.binarytree.main.IterativeBinaryTreeTraversal;

import java.util.ArrayList;
import java.util.List;

public class IterativeBinaryTreeTraversalTest {
  ...
  */
        ...
        IterativeBinaryTreeTraversal<Integer> treeTraversal = new IterativeBinaryTreeTraversal<>();

        List<Integer> inOrderRes = treeTraversal.inOrderIterativeTraversal(root);
        printRes(inOrder, inOrderRes, "In-Order");

        List<Integer> preOrderRes = treeTraversal.preOrderIterativeTraversal(root);
        printRes(preOrder, preOrderRes, "Pre-Order");

        List<Integer> postOrderRes = treeTraversal.postOrderIterativeTraversal_usingTwoStack(root);
        printRes(postOrder, postOrderRes, "Post-Order - Using Two Stack");

        List<Integer> postOrderRes_1 = treeTraversal.postOrderIterativeTraversal_usingOneStack(root);
        printRes(postOrder, postOrderRes_1, "Post-Order - Using One Stack");
